...
fn test_lender_consumer_observes_revocation_state() {
    ...
    let result_after =
        consumer_client.verify_with_safeguards(&lender, &business, &period, &merkle_root);
...
fn test_lender_consumer_observes_revocation_state_multi_period() {
    ...
    let res1_after =
        consumer_client.verify_with_safeguards(&lender, &business, &period1, &merkle_root1);
    assert!(!res1_after.is_valid);
    assert_eq!(res1_after.rejection_reason, REJECTION_REVOKED);

    let res2_after =
        consumer_client.verify_with_safeguards(&lender, &business, &period2, &merkle_root2);
