...
fn test_volume_bracket_exact_boundary() {
    ...
    for &(target_count, expected_bps) in boundary_cases.iter() {
        ...
        assert_eq!(
            t.client.get_volume_discount(&business),
            expected_bps,
            "Failed at count {}",
            target_count
        );
        assert_eq!(
            ...
            compute_fee(1_000_000, 0, expected_bps)
        );
    }
}
...
fn test_no_fee_config_free() {
    ...
    client.submit_attestation(
        &business, &period, &root, &1u64, &1u32, &0i128, &None, &None,
    );
...
fn discount_stacking_no_underflow() {
    ...
    let fee2 = compute_fee(500i128, 10_000u32, 10_000u32);
    assert!(
        fee2 >= 0,
    ...
    // Near-max discounts round to the smallest representable unit.
