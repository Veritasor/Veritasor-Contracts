        ...
        if let Some(expiry) = env.storage().instance().get::<_, u32>(&expiry_key) {
            if current_seq > expiry + grace {
                if let Some(proposal) =
                    env.storage().instance().get::<_, Proposal>(&MultisigKey::Proposal(id_u64))
                {
                    let action = proposal.action.clone();
                    let cleaned_at = env.ledger().sequence();
                    env.storage().instance().remove(&MultisigKey::Proposal(id_u64));
                    env.storage().instance().remove(&MultisigKey::Approvals(id_u64));
